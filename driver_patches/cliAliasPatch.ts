...
function applyReplacements(sourceFile: SourceFile, replacements: Array<[string, string]>) {
	...
	for (const [searchValue, replaceValue] of replacements) text = text.split(searchValue).join(replaceValue);
...
export function patchCliAlias(project: Project) {
	...
	applyReplacements(cliProgramSourceFile, [
		...
		[
			...
			"ensure browsers necessary for this version of Patchright are installed",
		],
		[
			"prints list of browsers from all playwright installations",
			"prints list of browsers from all patchright installations",
		],
		[
			"WARNING: It looks like you are running 'npx playwright install' without first",
			"WARNING: It looks like you are running 'npx patchright install' without first",
		],
		...
		[
			...
			"If your project does not yet depend on Patchright, first install the",
		],
		[
			"then run Playwright's install command to download the browsers:",
			"then run Patchright's install command to download the browsers:",
		],
		...
		[
			...
			"Removes browsers used by this installation of Patchright from the system (chromium, firefox, webkit, ffmpeg). This does not include branded channels.",
		],
		[
			"Removes all browsers used by any Playwright installation from the system.",
			"Removes all browsers used by any Patchright installation from the system.",
		],
		[
			"Successfully uninstalled Playwright browsers for the current Playwright installation.",
			"Successfully uninstalled Patchright browsers for the current Patchright installation.",
		],
		[
			"used by other Playwright installations.\\nTo uninstall Playwright browsers for all installations, re-run with --all flag.",
			"used by other Patchright installations.\\nTo uninstall Patchright browsers for all installations, re-run with --all flag.",
		],
		[
			"timeout for Playwright actions in milliseconds, no timeout by default",
			"timeout for Patchright actions in milliseconds, no timeout by default",
		],
	...
	// --------------------------
	const cliProgramWithTestStubSourceFile = project.addSourceFileAtPath(
	...
	applyReplacements(androidServerSourceFile, [["playwright install android", "patchright install android"]]);
	...
	applyReplacements(serverRegistrySourceFile, [
		[
			...
			"Looks like ${sdkLanguage === 'javascript' ? 'Patchright Test or ' : ''}Patchright was just updated to ${preferredDockerVersion.driverVersion}.",
		],
		[
			"Looks like ${sdkLanguage === 'javascript' ? 'Playwright Test or ' : ''}Playwright was just installed or updated.",
			"Looks like ${sdkLanguage === 'javascript' ? 'Patchright Test or ' : ''}Patchright was just installed or updated.",
		],
		["<3 Playwright Team", "<3 Patchright Team"],
		[
			"ERROR: Playwright does not support installing ${executable.name}",
			"ERROR: Patchright does not support installing ${executable.name}",
		],
		[
		...
		[
			...
			"ERROR: Patchright does not support ${descriptor.name} on ${hostPlatform}",
		],
		[
			"BEWARE: your OS is not officially supported by Playwright; downloading fallback build for ${hostPlatform}.",
			"BEWARE: your OS is not officially supported by Patchright; downloading fallback build for ${hostPlatform}.",
		],
