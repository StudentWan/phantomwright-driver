...
export function patchNetworkDispatchers(project: Project) {
	...
	const networkDispatchersSourceFile = project.addSourceFileAtPath(
	...
	const continueCall = assertDefined(
		continueMethod.getFirstDescendant(
			node => node.isKind(SyntaxKind.CallExpression) && node.getExpression().getText() === "this._object.continue",
		),
